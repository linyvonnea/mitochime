...
LITE mode (--lite) outputs only the 4 features used by your final Gradient Boosting model:
  - total_clipped_bases
  - kmer_js_divergence
  - kmer_cosine_diff
  - softclip_left

FULL mode (default) still exists if you ever need it, but your pipeline should use --lite.

Fixes included:
- Correct __main__ guard.
- SA strand comparison bug fixed (primary strand '+'/'-' for SA matching).
- --label optional (defaults -1 for inference).
"""
...
def kmer_profile(seq: str, k: int = 5) -> Dict[str, int]:
    seq = seq.upper()
    n = len(seq)
    counts: Dict[str, int] = {}
    if n < k:
...
def cosine_difference(p: Dict[str, float], q: Dict[str, float]) -> float:
    keys = set(p.keys()) | set(q.keys())
    if not keys:
        return 0.0
...
def parse_sa_tag(read: pysam.AlignedSegment) -> List[Dict]:
    ...
    for e in entries:
        rname, pos, strand, cigar, mapq, nm = e.split(",")
        segments.append(
            {"rname": rname, "pos": int(pos), "strand": strand, "cigar": cigar,
             "mapq": int(mapq), "nm": int(nm)}
        )
    return segments


def sa_feature_stats(primary_rname: str, primary_pos: int, primary_sa_strand: str,
                     sa_segments: List[Dict]) -> Dict:
    has_sa = 1 if sa_segments else 0
    sa_count = len(sa_segments)
    num_segments = 1 + sa_count
    ...
    for seg in sa_segments:
        ...
        if seg["rname"] != primary_rname:
            ...
            deltas_same.append(abs(seg["pos"] - primary_pos))

        if seg["strand"] == primary_sa_strand:
            same_strand += 1
        else:
            opp_strand += 1
...
def extract_features(bam_path: str, out_tsv: str, label: int,
                     k: int = 6, micro_window: int = 40, lite: bool = False) -> None:
    bam = pysam.AlignmentFile(bam_path, "rb")
    ...
    with open(out_tsv, "w", newline="") as f_out:
        ...
        if lite:
            ...
            writer.writerow([
                "read_id","label","read_length","mean_base_quality","ref_name","ref_start_1based",
                "strand","mapq","cigar",
                "has_sa","sa_count","num_segments","sa_diff_contig","sa_min_delta_pos","sa_max_delta_pos",
                "sa_mean_delta_pos","sa_same_strand_count","sa_opp_strand_count","sa_max_mapq","sa_mean_mapq",
                "sa_min_nm","sa_mean_nm",
                "softclip_left","softclip_right","total_clipped_bases",
                "breakpoint_read_pos","kmer_cosine_diff","kmer_js_divergence",
                "microhomology_length","microhomology_gc",
            ])
        ...
        for read in bam.fetch(until_eof=True):
            ...
            breakpoint = infer_breakpoint(read, read_len)
            breakpoint = max(0, min(breakpoint, read_len))

            left_probs = normalize_counts(kmer_profile(seq[:breakpoint], k=k))
            right_probs = normalize_counts(kmer_profile(seq[breakpoint:], k=k))
            ...
            ref_name = bam.get_reference_name(read.reference_id)
            ref_start_1based = read.reference_start + 1
            strand_num = 1 if read.is_reverse else 0
            primary_sa_strand = "-" if read.is_reverse else "+"

            mapq = read.mapping_quality
            cigar = read.cigarstring if read.cigarstring is not None else "*"

            sa_segments = parse_sa_tag(read)
            sa_feats = sa_feature_stats(ref_name, ref_start_1based, primary_sa_strand, sa_segments)

            left_window = seq[max(0, breakpoint - micro_window):breakpoint]
            right_window = seq[breakpoint:min(read_len, breakpoint + micro_window)]
            mh_len = longest_suffix_prefix_overlap(left_window, right_window, max_len=micro_window)
            mh_gc = gc_content(left_window[-mh_len:]) if mh_len > 0 else 0.0
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Extract features from a BAM file.")
    parser.add_argument("--bam", required=True, help="Input BAM (sorted).")
    parser.add_argument("--out", required=True, help="Output TSV.")
    parser.add_argument("--label", type=int, default=-1, help="0/1 for training; -1 for inference.")
    parser.add_argument("--k", type=int, default=6, help="k-mer size (use same as training).")
    parser.add_argument("--micro-window", type=int, default=40, help="microhomology window (use same as training).")
    parser.add_argument("--lite", action="store_true", help="Output only 4 GB features (recommended for inference).")
    args = parser.parse_args()
