...
if [[ -f "$SPADES_SCRIPT" ]]; then
  chmod +x "$SPADES_SCRIPT" >/dev/null 2>&1 || true
fi
...
if [[ "$DO_SPADES" =~ ^[Yy]$ ]]; then
  ...
  if [[ -f "$CNN_R1_OUT" && -f "$CNN_R2_OUT" ]]; then
    echo ""
  ...
  if [[ -f "$RNN_R1_OUT" && -f "$RNN_R2_OUT" ]]; then
    echo ""
