...
# ============================================================
# Batch Lemuru:
#   UNFILTERED + (GB + CNN) filtered across thresholds + SPAdes multi-k
#
# Canonical outputs (MATCHES your CLI + recommended pipeline scripts):
#  - GB writes to:  data/filtered_reads/${RUN}/${RUN}.gb.filtered_R{1,2}.fastq.gz
#  - CNN writes to: data/filtered_reads/${RUN}/${RUN}.cnn.filtered_R{1,2}.fastq.gz
# ============================================================
...
run_spades() {
  ...
  # If previous run failed (dir exists but no contigs), reset it.
  if [[ -d "$outdir" && ! -f "$outdir/contigs.fasta" ]]; then
    rm -rf "$outdir"
  fi
  mkdir -p "$outdir"
...
for ds in "${DATASETS[@]}"; do
  ...
  for thr in "${THRESHOLDS[@]}"; do
    ...
    # ---------- GB ----------
    GB_RUN="${ds}_gb_${ttag}"
    GB_OUTDIR="data/filtered_reads/${GB_RUN}"
    GB_R1_OUT="${GB_OUTDIR}/${GB_RUN}.gb.filtered_R1.fastq.gz"
    GB_R2_OUT="${GB_OUTDIR}/${GB_RUN}.gb.filtered_R2.fastq.gz"

    maybe_run "$GB_R1_OUT" \
      "bash \"$GB_SCRIPT\" \"$R1\" \"$R2\" \"$GB_RUN\" \"$thr\" \"$THREADS\" \"$REF_DEFAULT\""

    # assemble GB filtered (each klist)
    for k in "${KLISTS[@]}"; do
      ktag="k$(echo "$k" | tr ',' '_')"
    ...
    # assemble CNN filtered (each klist)
    for k in "${KLISTS[@]}"; do
      ktag="k$(echo "$k" | tr ',' '_')"
