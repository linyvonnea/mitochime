...
Building upon the principles of de novo chimera detection, Callahan et al. (2016) developed DADA2, a model-based approach that distinguishes true biological sequence variation from sequencing error at single-nucleotide resolution. DADA2 is an inference pipeline developed for Illumina data, particularly 16S rRNA gene sequencing. Unlike OTU-based methods, it infers ASVs through quality filtering, dereplication, denoising, paired-end merging, and chimera removal \citep{Callahan2016}.
...
Although DADA2 is effective for amplicon-based microbial studies, it is not designed for read-level detection of PCR-induced chimeras in single-species organellar sequencing data. Its dependence on ASV-level reconstruction and abundance relationships limits its direct applicability to the scope of the present study.
...
CNN has been widely used in genomic sequence analysis because they can learn local sequence patterns directly from encoded DNA or RNA inputs. In these models, convolutional filters slide across the sequence to detect recurring motifs, splice signals, binding sites, and short-range changes in nucleotide composition.
...
\citet{Shen2018} utilized this approach in developing KEGRU, a model designed to identify transcription factor binding sites in DNA sequences by combining k-mer embedding with a BiGRU. The results of the study showed better performance compared to other models that used CNN-based feature extraction.
