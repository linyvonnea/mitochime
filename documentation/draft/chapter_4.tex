...
The final dataset contained 31{,}986 reads for training and 7{,}997 reads for testing, with balanced classes (approximately 4{,}000 clean and 4{,}000 chimeric reads in the test split).
...
Features related to supplementary alignments show strong separation between classes. Chimeric reads frequently exhibit supplementary alignments, reflected by higher values of supplementary alignment presence, supplementary alignment count, and number of alignment segments, whereas clean reads consistently show a single alignment segment with no supplementary mappings. Table~\ref{tab:key_feature_summary} shows that supplementary alignment presence is present in chimeric reads (mean = 0.406) but absent in clean reads (mean = 0.000), while the number of alignment segments increases from a constant value of 1.000 in clean reads to a mean of 1.406 in chimeric reads. These patterns align with the expected structure of chimeric reads and indicate that alignment-based features are highly informative.

\subsubsection{Clipping-Based Features}

Clipping-related features, including left soft-clipped bases, right soft-clipped bases, and total clipped bases, display higher values and broader distributions in chimeric reads. In chimeric reads, total clipped bases reach 25.44 on average, with a median of 19.0 and an IQR of 48.0, while left soft-clipped bases and right soft-clipped bases have averages of 12.55 and 12.90, medians of 0.0, and IQRs of 19.0. Clean reads maintain values near zero across all these metrics. These patterns indicate substantial clipping and increased variability in chimeric reads, reflecting junction-like alignment fragmentation, whereas clean reads remain unaltered.

\subsubsection{K-mer Distribution Features}

K-mer--based features, including k-mer Jensen--Shannon divergence and k-mer cosine difference, show only minor differences between clean and chimeric reads. In chimeric reads, k-mer Jensen--Shannon divergence has a mean of 0.974 with a median of 0.986, and k-mer cosine difference has a mean of 0.974 with a median of 0.986. Clean reads show similar values, with k-mer Jensen--Shannon divergence at 0.976 with a median of 0.986, and k-mer cosine difference at 0.976 with a median of 0.986. The close similarity of the means, medians, and overall ranges of values indicates that these features alone provide limited ability to distinguish clean from chimeric reads.

\subsubsection{Microhomology Features}

Microhomology-related features, including microhomology length and microhomology GC content, exhibit nearly identical summary statistics between clean and chimeric reads. Most reads in both classes have short or zero-length microhomologies. Table~\ref{tab:key_feature_summary} shows that microhomology GC content has a mean of 0.172 and a median of 0.0 in both clean and chimeric reads, while microhomology length averages 0.458 with a median of 0.0 in chimeric reads and 0.462 with a median of 0.0 in clean reads. These values indicate that microhomology features alone provide limited discriminatory power and are more appropriately considered as supporting evidence.
...
For clipping-related features, left soft-clipped bases, right soft-clipped bases, and total clipped bases show higher medians and longer upper whiskers in chimeric reads than in clean reads, indicating increased variability and the presence of split alignments.

Supplementary alignment features, including supplementary alignment presence and supplementary alignment count, show that clean reads are largely zero, whereas chimeric reads display a wider distribution, reflecting frequent supplementary alignments.

K-mer metrics, including k-mer Jensen--Shannon divergence and k-mer cosine difference, show a slight upward shift for chimeric reads, but substantial overlap with clean reads indicates low discriminative power.

Microhomology features, including microhomology length and microhomology GC content, have nearly overlapping distributions for both classes, consistent with their low standalone predictive importance.
...
\caption{Confusion matrix of the tuned GB classifier on the held-out test set.}
\label{fig:cm_gradient_boosting}
\end{figure}

\subsection{ROC and Precision--Recall Curves}

The GB model achieved a ROC--AUC of approximately 0.84 and an average precision of approximately 0.88 on the held-out test set. The Precision--Recall (PR) curve shows that precision remains above 0.9 across a broad range of recall values up to roughly 0.5--0.6, after which precision gradually declines. This behaviour suggests that the classifier can assign very high confidence to a subset of chimeric reads, while the recovery of more ambiguous reads requires accepting lower precision. These curves indicate that GB provides a strong balance between sensitivity and specificity for the detection of chimeric reads.

\begin{figure}[H]
\centering
\includegraphics[width=0.8\linewidth]{figures/roc_pr_gradient_boosting.png}
\caption{ROC and precision--recall curves for the tuned GB classifier on the held-out test set.}
...
To understand how the final classifier made predictions, feature importance was quantified using permutation importance for the tuned GB model.

As shown in Figure~\ref{fig:gradient_imp}, total clipped bases provides the strongest predictive signal, with an importance of 0.117. This is followed by k-mer Jensen--Shannon divergence and k-mer cosine difference, indicating that both clipping-based alignment irregularities and k-mer compositional shifts are central to the model’s decision-making process. In contrast, soft-clipping features contribute less strongly, while microhomology-related features such as microhomology length and microhomology GC content provide minimal predictive value.
...
Feature selection was performed to identify the smallest subset reaching 95\% cumulative importance. Three configurations were evaluated as references: the full GB model with all 23 features, a reduced model with the top-\(k\) features, and an ablation model excluding microhomology features.

\subsection{Cumulative Importance Curve}

The cumulative importance curve was computed using the tuned Gradient Boosting classifier. Figure~\ref{fig:cumulative_importance} shows that the importance rises steeply for the top-ranked features and then gradually plateaus, indicating that a relatively small number of predictors account for most of the model’s performance. A cumulative importance threshold of 95\% was reached at \(k = 4\) features: total clipped bases, k-mer Jensen--Shannon divergence, k-mer cosine difference, and left soft-clipped bases.
...
Classification performance was compared across three feature sets using the tuned Gradient Boosting classifier. The full model, incorporating all 23 engineered features, achieved an F1-score of 0.7765 and a ROC--AUC of 0.8459. A reduced model using only the top four features, namely total clipped bases, k-mer Jensen--Shannon divergence, k-mer cosine difference, and left soft-clipped bases, achieved nearly equivalent performance, with an F1-score of 0.7768 and a ROC--AUC of 0.8369. An ablation model excluding microhomology features, namely microhomology length and microhomology GC content, also performed comparably, with an F1-score of 0.7761 and a ROC--AUC of 0.8444.
...
\midrule
Full Gradient Boost & 23 & 0.7765 & 0.8459 \\
...
\section{CNN Performance and Classification Results}
...
When evaluated on the held-out balanced test set, the final CNN achieved a test loss of 0.3034, accuracy of 0.9092, precision of 0.8724, recall of 0.9588, F1-score of 0.9135, and ROC--AUC of 0.9607, as shown in Table~\ref{tab:cnn_test_summary}. These results indicate that the model generalizes well beyond the validation folds and maintains strong classification performance on held-out test data.
...
\caption{BiGRU training and validation loss and validation F1 across epochs.}
\label{fig:rnn_training}
\end{figure}

On the held-out balanced test set, the final BiGRU model achieved a test loss of 0.2432, accuracy of 0.9516, precision of 0.9394, recall of 0.9655, F1-score of 0.9523, and ROC--AUC of 0.9849, as summarized in Table~\ref{tab:rnn_test_summary}.

\begin{table}[H]
\centering
\caption{Held-out test performance of the BiGRU model.}
...
\caption{Confusion matrix of the BiGRU model on the held-out test set.}
\label{fig:rnn_confusion}
\end{figure}

\section{External Assembly Testing}
...
Direct comparison of the three best-performing approaches, as summarized in Table~\ref{tab:gb_cnn_rnn_compare} and Figure~\ref{fig:gb_cnn_rnn_compare}, shows a clear difference in elapsed time, while the corresponding predictive performance metrics are presented in the table. The tuned GB model had the shortest runtime at 0.50 minutes, followed by the CNN at 1.39 minutes, while the RNN-kmer BiGRU had the longest runtime at 5.51 minutes. The reported CNN and RNN-kmer BiGRU runtimes were measured in a Kaggle environment using NVIDIA Tesla T4 x2 GPUs.
...
\midrule
GB (tuned) & 0.8091 & 0.9368 & 0.6630 & 0.7765 & 0.8459 & 0.50 \\
1D CNN                     & 0.9093 & 0.8724 & 0.9588 & 0.9135 & 0.9607 & 1.39 \\
BiGRU            & 0.9516 & 0.9394 & 0.9655 & 0.9523 & 0.9849 & 5.51 \\
\bottomrule
\end{tabular}
\end{table}
...
\subsection{Classifier Suitability for Pipeline Integration}

The comparative results indicate that classifier suitability for pipeline integration depends on the evaluation criterion considered. When read-level predictive performance is used as the primary basis, the BiGRU model performed strongest among the final models. It achieved the highest held-out accuracy, precision, recall, F1-score, and ROC--AUC, with an F1-score of 0.9523 and a ROC--AUC of 0.9849. These results suggest that sequential k-mer representations are highly effective for distinguishing clean and chimeric reads in the simulated mitochondrial dataset.

However, predictive performance alone did not fully determine practical suitability for deployment. Although Gradient Boosting produced lower read-level classification scores than the neural network models, it had the shortest runtime and offered greater interpretability because its predictions were based on explicit engineered features, including clipping patterns, supplementary alignment indicators, and k-mer discontinuity metrics. This makes Gradient Boosting a practical option for lightweight settings where model behaviour needs to be more easily explained.

The CNN showed a different practical advantage during downstream assembly testing. Although it did not achieve the highest read-level F1-score or ROC--AUC, it produced the simplest assembly graph in the 20K paired-end read dataset with 50\% chimeric reads. This result suggests that assembly improvement depends not only on overall classification performance, but also on which reads are removed and how much read support remains after filtering.

Taken together, the results show that no single classifier was uniformly superior across all evaluation criteria. Instead, the final models represented different trade-offs. BiGRU was most suitable when the priority was read-level classification accuracy, Gradient Boosting was most suitable when the priority was speed and interpretability, and CNN was useful under selected high-coverage and high-chimera conditions where stronger graph simplification was desired. This criterion-dependent behavior supports the modular design of MitoChime, where multiple classifier options can be used depending on the intended analysis context.