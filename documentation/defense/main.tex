...
\begin{columns}[T]
\column{0.33\textwidth}
\metriccard{Gradient Boosting}{Conservative}{Higher read retention}

\column{0.33\textwidth}
\metriccard{CNN}{Sensitive}{More aggressive filtering}

\column{0.33\textwidth}
\metriccard{BiGRU}{Highest F1}{Best read-level performance}
\end{columns}

\vspace{1.2em}

\begin{block}{Interpretation}
BiGRU achieved the best classification performance, but assembly outcomes also depended on how much read support remained after filtering.
\end{block}
...
Gradient Boosting missed more chimeras but avoided over-filtering, while CNN and BiGRU detected more chimeras but removed more reads.
\end{block}
...
\centering
\includegraphics[width=0.95\linewidth]{figures/gradient_imp_pair.png}

\column{0.42\textwidth}
\large
Most informative predictors:\\[0.5em]

\normalsize
\begin{itemize}
    \item total clipped bases,
    \item k-mer Jensen--Shannon divergence,
    \item k-mer cosine difference,
    \item left soft clipping.
\end{itemize}

\vspace{0.8em}

\begin{block}{Interpretation}
Chimeric reads were mainly identified through structural irregularities and sequence composition shifts.
\end{block}
...
\column{0.58\textwidth}
\centering
\includegraphics[width=0.95\linewidth]{figures/assembly_20k_50.pdf}
...
\column{0.58\textwidth}
\centering
\includegraphics[width=0.95\linewidth]{figures/assembly_3200_50.pdf}
...
\begin{columns}[T]
\column{0.48\textwidth}
\begin{block}{For practical use}
\begin{itemize}
    \item Use Gradient Boosting when read depth is low.
    \item Use BiGRU when read-level classification is the priority.
    \item Use CNN cautiously under low coverage.
\end{itemize}
\end{block}

\column{0.48\textwidth}
\begin{block}{For future work}
\begin{itemize}
