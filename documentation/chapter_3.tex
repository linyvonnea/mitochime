...
The microhomology length was computed as part of the feature extraction pipeline. For each aligned read in the BAM files, the script first inferred a breakpoint using the function \texttt{infer\_breakpoint}, which represents a junction between two segments. Breakpoints were determined primarily from soft-clipping patterns, where part of a read does not align to the reference and may indicate a junction. If no soft clips were present, SA tags were used to identify potential alignment discontinuities.

Once a breakpoint was established, the script scanned a $\pm 40$ base pair window surrounding the breakpoint and used the function \texttt{longest\_suffix\_prefix\_overlap} to identify the longest exact suffix-prefix overlap between the left and right read segments. This overlap, which represents consecutive bases shared at the junction, was recorded as the microhomology length. The 40-base pair window was chosen to ensure that short shared sequences at or near the breakpoint were captured, without including distant sequences that are unrelated. Additionally, the GC content of the overlapping sequence was calculated using the function \texttt{gc\_content}, which counts guanine (G) and cytosine (C) bases within the detected microhomology and divides by the total length, yielding a proportion between 0 and 1. Short microhomologies, typically 3-20 base pairs in length, are recognized signatures of PCR-induced template switching \citep{Peccoud2018}.

To provide additional sequence context around breakpoints, k-mer profiles were computed. K-mers are short sequences of consecutive nucleotides, and k-mer profiles quantify the frequency of each sequence. A k-mer length of 6 was used to capture patterns within the same 40-base pair window surrounding each breakpoint. High-frequency k-mers, such as repeated bases (e.g., "AAAAAA"), may indicate repetitive or unusual sequences at the junction. These profiles complement microhomology measurements and help identify junctions that are potentially chimeric.
...
\begin{table}[H]
  ...
  \resizebox{\textwidth}{!}{%
    \begin{tabular}{|c|c|c|c|c|c|c|c|}
      ...
      \hline
      Feature Extraction Pipeline & & & $\bullet\bullet\bullet\bullet$ & $\bullet\bullet\bullet\bullet$ &  &  &  \\
      \hline
