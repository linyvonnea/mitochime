...
\begin{frame}{Summary}
  \begin{itemize}
    \item Tree-based ensembles (CatBoost, Gradient Boosting, LightGBM) consistently outperform linear baselines.
    \item Best models reach F1 $\approx 0.77$ and ROC--AUC $\approx 0.84$ on held-out reads.
    \item Key predictive signals match chimera biology:
      \begin{itemize}
        \item k-mer composition jumps along the read
        \item extensive soft-clipping and total clipped bases
      \end{itemize}
    \item At the default threshold, the filter is conservative:
      \begin{itemize}
        \item preserves most clean reads
        \item removes a substantial fraction of chimeras
      \end{itemize}
    \item Overall: our feature set and model ensemble provide a practical pre-filter before mitochondrial assembly.
  \end{itemize}
\end{frame}
