...
\section{Convolutional Neural Network (CNN) Classification Performance}

The performance of the 1D Convolutional Neural Network (CNN) was evaluated on a held-out test set to determine its efficacy in classifying sequences as clean or chimeric. As shown in Figure~\ref{fig:cnn_train_test_loss}, the optimal model state was reached at epoch 25, where the test loss was minimized at 0.304. At this best epoch, the model achieved a test accuracy of 0.905 and a high Area Under the ROC Curve (AUC) of 0.958, indicating excellent separability between clean and chimeric reads across thresholds.

\begin{figure}[H]
  \centering
  \includegraphics[width=0.6\textwidth]{figures/cnn_train_test_loss.png} 
  \caption{Training and held-out test loss curves for the CNN model across epochs.}
  \label{fig:cnn_train_test_loss}
\end{figure}

\begin{figure}[H]
  \centering
  \includegraphics[width=0.6\textwidth]{figures/cnn_test_f1_auc.png} 
  \caption{Test-set F1-score and ROC--AUC per epoch for the CNN model}
  \label{fig:cnn_test_f1_auc}
\end{figure}

\begin{table}[htbp]
  \centering
  \small
  \caption{CNN performance on the test set at the best epoch.}
  \label{tab:cnn_metrics}
  \begin{tabular}{lrrrrrr}
    \toprule
    epoch & loss & acc & prec & rec & f1 & auc \\
    \midrule
    25 & 0.304093 & 0.904750 & 0.877918 & 0.940250 & 0.908015 & 0.957945 \\
    \bottomrule
  \end{tabular}
\end{table}

Class-wise performance indicates that the model is highly sensitive in correctly identifying chimeric reads, recording a recall of 0.940. This is further evidenced by the confusion matrix in Figure~\ref{fig:cnn_confusion_matrix}, which shows that the model correctly identified 3{,}761 chimeric sequences while only missing 239 (false negatives). While the precision was recorded at 0.878 due to 523 clean sequences being misclassified as chimeric (false positives), the resulting F1-score of 0.908 confirms that the model maintains a good balance between precision and recall. As illustrated in Figure~\ref{fig:cnn_test_f1_auc}, both the F1 and AUC scores stabilized early in training and remained consistently high through the final epochs.

\begin{figure}[H]
  \centering
  \includegraphics[width=0.6\textwidth]{figures/cnn_confusion_matrix.png}
  \caption{Confusion Matrix of CNN Classification Performance on Clean and Chimeric Reads}
  \label{fig:cnn_confusion_matrix}
\end{figure}

The stability of the CNN architecture was verified using five-fold cross-validation, as summarized in Figure~\ref{fig:cnn_cv}. The mean cross-validation scores---specifically an F1-score of $0.875 \pm 0.011$ and an AUC of $0.936 \pm 0.008$---align closely with the results obtained from the held-out test set. The low standard deviation across all folds suggests that the model is not overly sensitive to specific data partitions and possesses strong generalization capabilities for genomic sequence classification. 

\begin{figure}[H]
  \centering
  \includegraphics[width=0.6\textwidth]{figures/cnn_cv.png}
  \caption{Cross-Validation Results of CNN Classification Performance}
  \label{fig:cnn_cv}
\end{figure}

\section{Comparison of Model Performance}

To situate the result within the objective of the paper, the CNN's performance was compared against the best-performing traditional machine learning model, a tuned Gradient Boosting (GB) classifier. Table~\ref{tab:cnn_vs_gb} summarizes the results and shows that the CNN outperformed Gradient Boosting across all reported metrics, improving accuracy from 0.809 to 0.905, F1 from 0.777 to 0.908, and ROC--AUC from 0.846 to 0.958. Notably, while the Gradient Boosting model exhibited higher precision (0.937), its recall was substantially lower (0.663), indicating a conservative decision strategy that reduced false positives at the expense of missed chimeric reads. In contrast, the CNN maintained high recall (0.940) while preserving strong precision (0.878), yielding a more favorable balance for detecting chimeric reads.

\begin{table}[htbp]
  \centering
  \small
  \caption{Performance comparison between the tuned Gradient Boosting classifier and the 1D CNN on the test set.}
  \label{tab:cnn_vs_gb}
  \begin{tabular}{lrrrrr}
    \toprule
    model & acc & prec & rec & f1 & auc \\
    \midrule
    GradientBoosting (tuned) & 0.809053 & 0.936771 & 0.663000 & 0.776460 & 0.845858 \\
    CNN1D (final held-out test) & 0.904750 & 0.877918 & 0.940250 & 0.908015 & 0.957945 \\
    \bottomrule
  \end{tabular}
\end{table}
...
Table~\ref{tab:external_results} shows the external assembly testing evaluated whether filtering removed simulated chimeric reads while preserving assembly capability. Assemblies were generated from datasets with 10\%, 20\%, and 50\% chimera rates. For the 10\% and 20\% datasets, all filters produced complete assemblies with a single scaffold and one graph node, and assembly sizes remained stable at 16,612--16,615~bp, matching the unfiltered baseline.  

Read retention varied by filtering strategy but remained sufficient for successful assembly. Gradient Boosting retained 95.47\% of reads in the 10\% chimera rate dataset and 91.26\% in the 20\% dataset. CNN filtering retained 77.34\% (10\%) and 70.59\% (20\%), while the transformer filter retained 81.89\% and 75.61\% in the 10\% and 20\% datasets respectively. Despite these reductions, assemblies remained structurally simple with a single scaffold and one graph node, indicating that the remaining reads were adequate for mitochondrial sequence reconstruction. 

The 50\% dataset required greater filtering because of the high proportion of chimeric reads. Unfiltered assemblies showed fragmentation with up to two scaffolds and three graph nodes. After filtering, assemblies were simplified, having one scaffold and one node, and sequence recovery remained complete (16,543--16,763~bp depending on the filter). These results demonstrate that filtering removes chimeric reads while preserving mitochondrial assembly capability.
...
\toprule
Filter & Chimera\% & Reads retained\% & Scaffolds & Graph nodes & Assembly bp \\
\midrule
CNN & 10\% & 77.34\% & 1 & 1 & 16612 \\
Gradient Boosting & 10\% & 95.47\% & 1 & 1 & 16612 \\
Transformer & 10\% & 81.89\% & 1 & 1 & 16612 \\
Unfiltered & 10\% & 100\% & 1 & 1 & 16612 \\
\midrule
CNN & 20\% & 70.59\% & 1 & 1 & 16615 \\
Gradient Boosting & 20\% & 91.26\% & 1 & 1 & 16597 \\
Transformer & 20\% & 75.61\% & 1 & 1 & 16615 \\
Unfiltered & 20\% & 100\% & 1 & 1 & 16669 \\
\midrule
CNN & 50\% & 48.57\% & 1 & 1 & 16543 \\
Gradient Boosting & 50\% & 79.75\% & 2 & 2 & 16763 \\
Transformer & 50\% & 56.48\% & 1 & 1 & 16623 \\
Unfiltered & 50\% & 100\% & 2 & 3 & 16680 \\
\bottomrule
\end{tabular}%
}
\caption{External assembly testing results}
