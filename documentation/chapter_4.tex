...
To evaluate broader predictive patterns, features were grouped into five families: 
SA\_structure, Clipping, Kmer\_jump, Micro\_homology, and Other.
...
Taken together, both feature-level and feature-family analyses indicate that Gradient Boosting relies primarily on breakpoint-associated clipping irregularities and shifts in local sequence composition to distinguish chimeric from clean reads.
...
Figure~\ref{fig:cnn_training} illustrates the learning behaviour of the model across epochs. Training loss decreases steadily, while validation loss declines during the early epochs and then stabilizes with only small fluctuations. Validation F1 follows a similar pattern, rising early in training before reaching a plateau, which suggests that the network learns discriminative patterns quickly and then converges to a stable performance region.
...
% Taken together, these results show that the CNN is a reliable model for chimera detection and performs well under both cross-validation and held-out test evaluation. Its strongest characteristic is its ability to detect positive cases with high sensitivity, making it a useful approach when minimizing missed chimeric reads is an important objective.
